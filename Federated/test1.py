...
from augment import random_walk_embedding_augmentation  # Import the correct augmentation function
...
# Embedding Augmentation Function
def augment_embeddings(embeddings, labels, augment_function):
    ...
    for embedding, label in zip(embeddings, labels):
        if np.sum(label) == 0:  # Skip if no active label
            continue
        aug_embedding, aug_label = augment_function(embedding, label)
        augmented_embeddings.append(aug_embedding)
    ...
    print(f"Final augmented labels shape: {augmented_labels.shape}")
...
def train_with_embedding_switch(model, embedding_model, train_data, train_y_tensor, test_data, test_y_tensor, augment_function, batch_size=32):
    ...
    model.fit(train_data, train_y_tensor, epochs=50, batch_size=batch_size, validation_data=(test_data, test_y_tensor))  # Adjust epochs as needed
    ...
    # Step 3: Augment embeddings
    print("Augmenting embeddings...")
    augmented_embeddings, augmented_labels = augment_embeddings(embeddings, train_y_tensor, augment_function)
...
class MyClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # Train with embedding switch
        history = train_with_embedding_switch(model, embedding_model, train_data, train_y_tensor, test_data, test_y_tensor, random_walk_embedding_augmentation)
